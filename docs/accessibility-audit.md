...
4. **`js-defi` template** doesn't ship `NetworkSwitcher`/`AccountSwitcher` at all (by design — it's an intentionally reduced template), so the fixes above don't apply there; worth a follow-up check whenever those components are added to a template. This was since resolved for the other reduced template: `minimal` and `js-minimal` now ship `NetworkSwitcher`, and both use the `labelId` + `aria-labelledby` approach with an `sr-only sm:not-sr-only` label, which keeps the accessible name present below the `sm` breakpoint without the hidden-label gap the `aria-label` fix above addressed.
