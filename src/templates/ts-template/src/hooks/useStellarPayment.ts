...
export function useStellarPayment(
   ...
   */
  const getNetworkPassphrase = useCallback((): string => {
   ...
   */
  const isValidSecret = useCallback((secret: string): boolean => {
   ...
   */
  const validatePaymentParams = useCallback((params: PaymentParams): void => {
   ...
   */
  const buildPaymentXDR = useCallback(async (params: PaymentParams): Promise<string> => {
   ...
   */
  const submitSignedXDR = useCallback(async (signedXdrBase64: string): Promise<PaymentResult> => {
    if (!serverRef.current) {
      throw new Error('Horizon server not initialized');
    }
   ...
   * ```
   */
  const signAndSubmitWithSecret = useCallback(async (
