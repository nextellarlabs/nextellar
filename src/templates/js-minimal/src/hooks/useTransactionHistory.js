 ...
 *
 * @param {string|null} [publicKey] - Stellar public key to fetch history for (optional)
 * @param {object} [options] - Configuration options including Horizon URL, page size, and type
 * @param {string} [options.horizonUrl] - Horizon server URL (falls back to the wallet provider config)
 * @param {number} [options.pageSize] - Number of records fetched per page
 * @param {'payments'|'operations'} [options.type] - Which Horizon endpoint to page through
 * @returns {{
 *   items: Array<object>,
 *   loading: boolean,
 *   error: Error|null,
    ...
    async (key, cursor) => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `Network error: ${errorObj.message || 'Failed to connect to Horizon'}`,
          );
        }
        ...
        if (
          ...
          throw new Error(
            `Client error: ${errorObj.message || 'Invalid request to Horizon'} (Status: ${errorObj.response.status})`,
          );
        }
        ...
        throw err instanceof Error
          ? err
          : new Error('Unknown error fetching transaction history');
      }
    },
    ...
    try {
      ...
      setItems((prevItems) => {
        ...
        if (newItems.length > MAX_ITEMS_IN_MEMORY) {
          ...
          console.warn(
            `Transaction history trimmed to ${MAX_ITEMS_IN_MEMORY} items to prevent excessive memory usage`,
          );
          return trimmedItems;
        }
