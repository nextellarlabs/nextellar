...
const TRANSACTION_RESULT_CODES: Record<string, string> = {
...
const OPERATION_RESULT_CODES: Record<string, string> = {
...
export interface DecodedResultCodes {
  /** Raw transaction-level code, e.g. "tx_failed". */
  transactionCode?: string;
  /** Raw per-operation codes, in operation order. */
  operationCodes?: string[];
  /**
   * The single most specific reason available: the first non-success
   * operation code's description if present, otherwise the transaction
   * code's description, otherwise a generic fallback.
   */
  reason: string;
}
