...
export default function AccountSwitcher() {
    ...
    const handleAccountChange = async (address) => {
        if (address !== publicKey) await switchAccount(address);
        setIsOpen(false);
    };
    ...
    return (
        <div ref={dropdownRef} className="relative inline-block">
            ...
            {isOpen && (
                <div id="account-switcher-menu" role="menu" className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50">
                    <div className="p-2">
                        <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 px-3 py-2">Available Accounts ({accounts.length})</div>
                        {accounts.map((account) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 rounded-md transition-colors flex items-center gap-2 ${isCurrent ? 'bg-blue-100 dark:bg-blue-900 text-blue-900 dark:text-blue-100' : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-900 dark:text-gray-100'}`}
                                >
                                    <span className="flex-1">
                                        <div className="font-medium">{account.displayName || account.address}</div>
                                        {account.displayName && <div className="text-xs text-gray-500 dark:text-gray-400">{account.address}</div>}
                                    </span>
                                    {isCurrent && (
