...
/** Convert a stroops string to a human-readable XLM amount (7 decimal places). */
...
export interface SimulationPreview {
  /** Decoded return value from the contract simulation. */
  result: unknown;
  /**
   * Minimum resource fee in stroops, as reported by the Soroban RPC.
...
export interface ContractCallPreviewProps {
  /**
   * Simulation data to display. When `undefined` and `loading` is `false`,
   * the component renders nothing (caller controls visibility).
   */
  preview?: SimulationPreview;
  /** True while the simulation RPC call is in-flight. */
  loading?: boolean;
  /** Error thrown by `simulateContractCall`, if any. */
  error?: Error | null;
  /** Called when the user confirms they want to submit the transaction. */
  onConfirm?: () => void;
  /** Called when the user cancels and goes back to editing. */
  onCancel?: () => void;
  /** Extra CSS classes applied to the root element for layout composition. */
  className?: string;
}
 ...
 *
 * Intended to be rendered just above the submit button in a `ContractCallForm`,
 * so users can review costs and expected output before spending fees.
 *
 * @example
 * ```tsx
 * const { simulateContractCall, loading, error } = useSorobanContract({ contractId });
 * const [preview, setPreview] = useState<SimulationPreview>();
 *
 * const handlePreview = async () => {
 *   const result = await simulateContractCall('transfer', ['GABC...', 100]);
 *   setPreview(result);
 * };
 *
 * return (
