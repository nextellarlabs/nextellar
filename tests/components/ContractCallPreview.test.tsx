...
describe("ContractCallPreview", () => {
  ...
  describe("loading state", () => {
    it("renders a live region with accessible label while loading", () => {
      ...
      const region = screen.getByRole("status", {
        name: /simulating contract call/i,
      });
  ...
  describe("error state", () => {
    ...
    it("displays the error message", () => {
      render(<ContractCallPreview error={err} />);
      expect(
        screen.getByText(/contract reverted with panic/i),
      ).toBeInTheDocument();
    });
    ...
    it("does not render the preview section when there is an error", () => {
      render(<ContractCallPreview error={err} preview={BASE_PREVIEW} />);
      expect(
        screen.queryByRole("region", { name: /simulation preview/i }),
      ).toBeNull();
    });
  ...
  describe("preview data", () => {
    ...
    it("displays the string return value", () => {
      render(
        <ContractCallPreview preview={{ ...BASE_PREVIEW, result: "ok" }} />,
      );
    ...
    it('displays "(void)" for a null return value', () => {
      render(
        <ContractCallPreview preview={{ ...BASE_PREVIEW, result: null }} />,
      );
    ...
    it("displays a boolean return value", () => {
      render(
        <ContractCallPreview preview={{ ...BASE_PREVIEW, result: true }} />,
      );
  ...
  describe("action buttons", () => {
    it("renders Confirm & Submit button when onConfirm provided", () => {
      render(
        <ContractCallPreview preview={BASE_PREVIEW} onConfirm={jest.fn()} />,
      );
      expect(
    ...
    it("calls onConfirm when Confirm & Submit is clicked", () => {
      ...
      render(
        <ContractCallPreview preview={BASE_PREVIEW} onConfirm={onConfirm} />,
      );
    ...
    it("renders Cancel button when onCancel provided", () => {
      render(
        <ContractCallPreview preview={BASE_PREVIEW} onCancel={jest.fn()} />,
      );
      expect(
    ...
    it("calls onCancel when Cancel is clicked", () => {
      const onCancel = jest.fn();
      render(
        <ContractCallPreview preview={BASE_PREVIEW} onCancel={onCancel} />,
      );
  ...
  describe("result formatting", () => {
    it('renders undefined result as "(void)"', () => {
      render(
        <ContractCallPreview
          preview={{ ...BASE_PREVIEW, result: undefined }}
        />,
      );
      expect(screen.getByText("(void)")).toBeInTheDocument();
    });
  ...
  describe("base fee", () => {
    it("does not render a base fee row when baseFee is omitted", () => {
      render(<ContractCallPreview preview={BASE_PREVIEW} />);
      expect(screen.queryByText(/base fee/i)).toBeNull();
      expect(screen.queryByText(/total fee/i)).toBeNull();
    });

    it('keeps the "Estimated fee" label when baseFee is omitted', () => {
      render(<ContractCallPreview preview={BASE_PREVIEW} />);
      expect(screen.getByText(/Estimated fee/i)).toBeInTheDocument();
      expect(screen.queryByText(/^Resource fee$/i)).toBeNull();
    });

    it("renders the base fee row inside the preview section when baseFee is provided", () => {
      render(
        <ContractCallPreview preview={{ ...BASE_PREVIEW, baseFee: "100" }} />,
      );
      // Reuses this file's existing getPreviewSection helper to confirm the
      // base fee row renders inside the same preview <section>, not as a
      // separate, disconnected element.
      const section = getPreviewSection();
      expect(within(section).getByText(/Base fee/i)).toBeInTheDocument();
    });

    it("relabels the resource fee row when baseFee is provided", () => {
      render(
        <ContractCallPreview preview={{ ...BASE_PREVIEW, baseFee: "100" }} />,
      );
      expect(screen.getByText(/^Resource fee$/i)).toBeInTheDocument();
      expect(screen.queryByText(/^Estimated fee$/i)).toBeNull();
    });

    it("renders the total fee as the sum of base fee and resource fee", () => {
      render(
        // 100 (base) + 500 (resource) = 600
        <ContractCallPreview preview={{ ...BASE_PREVIEW, baseFee: "100" }} />,
      );
    ...
    it("converts the total fee to its XLM equivalent", () => {
      render(
        <ContractCallPreview
          preview={{
            ...BASE_PREVIEW,
      ...
      // 20,000,000 stroops = 2 XLM
      expect(screen.getByText(/2\.0000000/)).toBeInTheDocument();
    });
  });
});
