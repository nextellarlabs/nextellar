 ...
 *
 * BalanceDisplay Component Tests — js-template (#839)
 */
...
jest.unstable_mockModule(
  "../../src/templates/js-template/src/hooks/useStellarBalances",
  () => ({
...
const [{ default: BalanceDisplay }, { useStellarBalances }, { useWallet }] =
  await Promise.all([
    import("../../src/templates/js-template/src/components/BalanceDisplay"),
    import("../../src/templates/js-template/src/hooks/useStellarBalances"),
...
describe("BalanceDisplay (js-template)", () => {
  ...
  it("renders without crashing when called with no props at all", () => {
    mockWallet({ connected: false, publicKey: undefined });
    mockBalances();
    expect(() => render(<BalanceDisplay />)).not.toThrow();
  });
});
