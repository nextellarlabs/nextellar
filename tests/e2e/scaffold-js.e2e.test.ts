...
describeE2E("e2e: --javascript scaffold", () => {
  ...
  it("scaffolds, has no stray TS files, and builds successfully", async () => {
    const parentDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nextellar-e2e-js-"),
    );
  ...
  it("scaffolds --template minimal --javascript and builds successfully", async () => {
    const parentDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nextellar-e2e-js-minimal-"),
    );
    tmpDirs.push(parentDir);
    ...
    // Placeholders resolved in the JS source files
    const pkgJson = await fs.readJson(path.join(appDir, "package.json"));
    expect(pkgJson.name).toBe(appName);
    ...
    expect(walletKit).not.toContain("{{NETWORK}}");
    expect(walletKit).not.toContain("{{WALLETS}}");

    // Minimal's expected JS hooks are present
    const hookFiles = await fs.readdir(path.join(appDir, "src/hooks"));
    expect(hookFiles).toContain("useStellarWallet.js");
    expect(hookFiles).toContain("useStellarBalances.js");
