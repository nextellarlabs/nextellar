...
describe("deploy bundle (#669)", () => {
  ...
  const makeProject = async (): Promise<string> => {
    ...
    await fs.outputFile(
      path.join(dir, ".nextellar/deploy/old-bundle.tar.gz"),
      "stale",
    );
  ...
  const listEntries = async (bundlePath: string): Promise<string[]> => {
    ...
    await list({
      ...
      onentry: (e) =>
        entries.push(
          e.path.replace(/\\/g, "/").replace(/^\.\//, "").replace(/\/$/, ""),
        ),
    });
  ...
  it("excludes node_modules, .git, .next/cache and .nextellar/deploy", async () => {
    ...
    for (const entry of entries) {
      for (const pattern of excluded) {
        expect(entry === pattern || entry.startsWith(`${pattern}/`)).toBe(
          false,
        );
      }
    }
  ...
  it("round-trips: the bundle extracts and file contents match", async () => {
    ...
    const extractDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nextellar-deploy-extract-"),
    );
    tmpDirs.push(extractDir);
    await extract({ file: bundlePath, cwd: extractDir });

    expect(
      await fs.readFile(path.join(extractDir, "src/index.ts"), "utf8"),
    ...
    expect(await fs.pathExists(path.join(extractDir, "node_modules"))).toBe(
      false,
    );
  });
});

// #1058 — large bundle uploads (e.g. a project's compiled contracts/ output)
// previously gave no feedback until the archive finished. onProgress fires a
// callback per packed file so a caller (CLI progress bar, tests) can observe
// bytes/files packed as it happens.
describe("deploy bundle progress reporting (#1058)", () => {
  const tmpDirs: string[] = [];
  let logSpy: ReturnType<typeof jest.spyOn> | undefined;

  const makeProject = async (): Promise<string> => {
    const dir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nextellar-deploy-progress-"),
    );
    tmpDirs.push(dir);
    ...
    // Simulates a compiled contract's WASM output — the motivating case for
    // progress reporting on "large" uploads.
    await fs.outputFile(
      path.join(dir, "contracts", "hello_world.wasm"),
      Buffer.alloc(2048, 1),
    );
    await fs.outputFile(
      path.join(dir, "src/index.ts"),
    ...
    // Present to prove exclusions never fire onProgress either.
    await fs.outputFile(
      path.join(dir, "node_modules/left-pad/index.js"),
      "junk",
    );
  ...
  it("fires onProgress once per packed file, in the same order tar writes entries", async () => {
    const projectDir = await makeProject();
    const updates: Array<{ filesPacked: number; lastEntry: string }> = [];

    await runDeploy({
      cwd: projectDir,
      onProgress: (progress) => {
        updates.push({
          filesPacked: progress.filesPacked,
    ...
    // filesPacked is a strictly increasing running count, one per call.
    expect(updates.map((u) => u.filesPacked)).toEqual(
      Array.from({ length: updates.length }, (_, i) => i + 1),
    );
    expect(updates.map((u) => u.lastEntry)).toEqual(
      expect.arrayContaining([
        "package.json",
        "src/index.ts",
        "contracts/hello_world.wasm",
      ]),
    );
  });

  it("never reports progress for excluded paths (node_modules)", async () => {
    const projectDir = await makeProject();
    const entries: string[] = [];

    await runDeploy({
      cwd: projectDir,
      onProgress: (progress) => entries.push(progress.lastEntry),
    });

    expect(entries.some((e) => e.startsWith("node_modules"))).toBe(false);
  });

  it("reports totalFiles and totalBytes up front, known before the first callback", async () => {
    const projectDir = await makeProject();
    const updates: Array<{ totalFiles: number; totalBytes: number }> = [];
    ...
    expect(updates.length).toBeGreaterThan(0);
    // totalFiles/totalBytes are the same fixed denominator on every call.
    const distinctTotals = new Set(
      updates.map((u) => `${u.totalFiles}:${u.totalBytes}`),
    );
    expect(distinctTotals.size).toBe(1);
    expect(updates[0].totalFiles).toBe(updates.length);
    expect(updates[0].totalBytes).toBeGreaterThanOrEqual(2048); // at least the WASM fixture's size
  });

  it("accumulates bytesPacked monotonically up to totalBytes on the final call", async () => {
    const projectDir = await makeProject();
    const updates: Array<{ bytesPacked: number; totalBytes: number }> = [];

    await runDeploy({
      cwd: projectDir,
      onProgress: (progress) => {
        updates.push({
          bytesPacked: progress.bytesPacked,
          totalBytes: progress.totalBytes,
        });
      },
    });
    ...
    for (let i = 1; i < updates.length; i++) {
      expect(updates[i].bytesPacked).toBeGreaterThanOrEqual(
        updates[i - 1].bytesPacked,
      );
    }
    const last = updates[updates.length - 1];
    expect(last.bytesPacked).toBe(last.totalBytes);
  });

  it("does not scan the project (no totals computed) when onProgress is omitted", async () => {
    const projectDir = await makeProject();
    // No onProgress passed — should behave exactly like the pre-#1058 path.
    await expect(runDeploy({ cwd: projectDir })).resolves.toBeUndefined();
  ...
  it("does not fire onProgress in --dry-run mode (no bundle is created)", async () => {
    const projectDir = await makeProject();
    const onProgress = jest.fn();

    await runDeploy({ cwd: projectDir, dryRun: true, onProgress });
...
describe("runDeploy validation & state (#641)", () => {
  ...
  const makeTempDir = async (): Promise<string> => {
    const dir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nextellar-deploy-state-"),
    );
    tmpDirs.push(dir);
    return dir;
  };
  ...
  it("rejects a directory without package.json", async () => {
    const dir = await makeTempDir();
    await expect(runDeploy({ cwd: dir })).rejects.toThrow(
  ...
  it("--dry-run performs no writes (no .nextellar/deploy/ is created)", async () => {
    ...
    expect(await fs.pathExists(path.join(dir, ".nextellar", "deploy"))).toBe(
      false,
    );
  });
